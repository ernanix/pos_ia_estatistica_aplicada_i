...
***Sobre a resposta:*** Em um primeiro levantamento, verificamos que as variáveis "age","parea","tarea","ensuit","garag","elev","fitg" e "categ" possui maiores graus de significância, ou seja, estatisticamente diferentes de zero. R² neste caso possui o valor de 0.8099, ou seja, as variáveis explicativas conseguem explicar 80,99% das variações da variável dependente ("price"). No caso do R² ajustado, o valor vai para 80,3% O valor da estatística F de 116.8 informa que pelo menos um dos parâmetros é diferente de 0, afirmando que existe uma reta de regressão
...
***Sobre a resposta:*** O F Calculado é de 4.3391, ou seja, maior que o F Tabelado. Nesse caso, ajustamos as variáveis de acordo com os testes em b)
...
```
***Sobre a resposta:*** Mesmo realizando as transformações nas variáveis, conforme apresentado nos testes em b), o F calculado foi de 4.5847, ou seja, um valor levemente maior que o encontrado anteriormente. Tentaremos baixar este valor realizando os testes de multicolinearidade e exclusão de possíveis variáveis.
...
***Sobre a resposta:*** Pelo teste VIF, verificou-se valores superiores em 'tarea' e 'parea', o que leva a entender que são altamente correlacionadas. Aplicou-se então a exclusão de 'tarea'. Pela Matriz de Correlação, percebe-se correlações entre as variaveis 'parea','bath','ensuit' e 'garag'. Optou-se por retirar estas 3 últimas.
...
***Sobre a resposta:*** Segundo a técnica de stepwise, o modelo com o menor AIC,e portanto o melhor modelo considerado, é o price ~ age + parea + plaz + park + kidca + bike + balc + elev + fitg + party + categ. 

#### Retestando o novo modelo
```{r, echo=FALSE}
formBase = formula(price~age+parea+plaz+park+kidca+bike+balc+elev+fitg+party+categ)
resultados <- lm(formula=formBase,data=imoveiscwbav)
...
```
***Sobre a resposta:*** Após as alterações no modelo definidas pelo stepwise, o F caiu para 3.9724
...
```
***Sobre a resposta:*** o resultado do teste BP foi de 199.49. Por ser maior que o valor chiquadrado tabelado, rejeita-se a hipótese de homocedasticidade
...
```
***Sobre a resposta:*** Para o modelo de regressão normal, tem-se os valores de RMSE e Sigma ligeiramente menores, se comparado os mesmos indicadores para o modelo robusto. No caso do R², o valor é 0,002 maior. Sendo assim, opta-se por continuar com a regressão normal.


#### **h) Estime os intervalos de confiança para os parâmetros do modelo**
...
+ kidca = 1.400 -> log(1.400) = 0.3362
+ bike = 0.3489 -> log(0.3489) = -1.0531
+ balc = 0
+ elev = 0
+ fitg = 0
+ party = 1
